...
export function LearningStudio({ language, latestAssistantMessage, onAskEdu, onClose, responseStyle, onResponseStyleChange, chatState, onRestoreWorkspace, accountConnected, onConnectAccount }: LearningStudioProps) {
  ...
  const accountWorkspaceSync = trpc.workspace.accountSync.useMutation();
  ...
  const uploadAccountWorkspace = async () => {
    if (!accountConnected) return;
    if (!persistSyncCode(syncCode)) { setSyncStatus(language === "es" ? "Crea o introduce tu código privado antes de guardar en la cuenta." : language === "ru" ? "Создайте или введите личный код перед сохранением в аккаунте." : "Create or enter your private code before saving to your account."); return; }
    try {
      setSyncStatus(language === "es" ? "Cifrando y guardando en tu cuenta…" : language === "ru" ? "Шифрование и сохранение в аккаунте…" : "Encrypting and saving to your account…");
      const encrypted = await encryptWorkspace(syncCode, snapshot());
      await accountWorkspaceSync.mutateAsync({ action: "put", ciphertext: encrypted.ciphertext });
      setSyncStatus(language === "es" ? "Copia cifrada guardada en tu cuenta. Tu código sigue siendo necesario para abrirla." : language === "ru" ? "Зашифрованная копия сохранена в аккаунте. Ваш код всё ещё нужен для открытия." : "Encrypted copy saved to your account. Your code is still required to open it.");
    } catch { setSyncStatus(language === "es" ? "No se pudo guardar la copia en tu cuenta ahora." : language === "ru" ? "Не удалось сохранить копию в аккаунте сейчас." : "Your account copy could not be saved right now."); }
  };
  const restoreAccountWorkspace = async () => {
    if (!accountConnected) return;
    const token = parseSyncCode(syncCode);
    if (!token || !persistSyncCode(syncCode)) { setSyncStatus(language === "es" ? "Introduce el código privado que protege tu copia de cuenta." : language === "ru" ? "Введите личный код, защищающий копию аккаунта." : "Enter the private code that protects your account copy."); return; }
    try {
      setSyncStatus(language === "es" ? "Buscando y descifrando tu copia de cuenta…" : language === "ru" ? "Поиск и расшифровка копии аккаунта…" : "Finding and decrypting your account copy…");
      const remote = await accountWorkspaceSync.mutateAsync({ action: "get" });
      if (!remote.found || !remote.ciphertext) { setSyncStatus(language === "es" ? "Aún no hay una copia guardada en esta cuenta." : language === "ru" ? "В этом аккаунте ещё нет сохранённой копии." : "There is no copy saved to this account yet."); return; }
      const restored = await decryptWorkspace(syncCode, remote.ciphertext);
      if (!isWorkspaceSnapshot(restored)) throw new Error("Invalid workspace");
      onRestoreWorkspace(restored); setNotes(restored.learning.notes); setReadingList(restored.learning.readingList); setProgress(restored.learning.progress);
      setSyncStatus(language === "es" ? "Copia de cuenta recuperada de forma privada." : language === "ru" ? "Копия аккаунта восстановлена конфиденциально." : "Account copy restored privately.");
    } catch { setSyncStatus(language === "es" ? "No fue posible abrir la copia de cuenta. Verifica tu código." : language === "ru" ? "Не удалось открыть копию аккаунта. Проверьте код." : "The account copy could not be opened. Check your code."); }
  };
