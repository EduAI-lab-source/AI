import { AlignLeft, Clock3, Download, FileAudio, LoaderCircle, Play, Sparkles, Volume2, Wand2 } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import type { AppLanguage } from "@/lib/i18n";
import { TTS_DAILY_CHARACTER_LIMIT, TTS_MAX_CHARACTERS, countTtsWords, formatTtsDuration, getTtsApiUrl, getTtsError, getTtsVisitorId, normalizeTtsText, prepareTtsText } from "@/lib/ttsRuntime";
...
export function TextToSpeechStudio({ language, latestAssistantMessage }: { language: AppLanguage; latestAssistantMessage?: string }) {
  ...
  const wordCount = useMemo(() => countTtsWords(text), [text]);
  ...
  const generateAudio = async () => {
    ...
    setIsGenerating(true); setStatus("");
    try {
      const response = await fetch(getTtsApiUrl(), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ text: cleanText, speaker, visitorId, turnstileToken }) });
    ...
    finally { setIsGenerating(false); setTurnstileToken(""); if (turnstileWidgetRef.current) window.turnstile?.reset(turnstileWidgetRef.current); }
  };

  return <section className="tts-studio" aria-labelledby="tts-title">
    <div className="tts-intro"><p className="tts-overline"><Sparkles size={13} /> {copy.eyebrow}</p><h2 id="tts-title">{copy.title}<br /><em>{copy.emphasis}</em></h2><p>{copy.description}</p>{latestAssistantMessage && <button className="tts-use-response" onClick={useLatestResponse}><FileAudio size={14} />{copy.useResponse}</button>}</div>
