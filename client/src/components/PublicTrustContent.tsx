  ...
  es: {
    privacy: {
      ...
      sections: [
        ...
        { heading: "Cookies, almacenamiento y terceros", body: ["Edu AI utiliza almacenamiento local esencial para conservar tus preferencias y tu trabajo en el dispositivo. La medición técnica del dominio se realiza con Cloudflare Web Analytics. El navegador también puede cargar la etiqueta oficial de Google AdSense asociada al dominio para preparar y verificar la plataforma; Edu AI no configura bloques publicitarios ni activa publicidad personalizada desde la interfaz.", "Si en el futuro se habilitan anuncios o analítica opcional, esta página se actualizará antes de activar esas tecnologías y se mostrará una elección de consentimiento cuando corresponda. Google puede tratar información técnica conforme a sus propias políticas al cargar su etiqueta."] },
  ...
  en: {
    privacy: {
      ...
      sections: [
        ...
        { heading: "Cookies, storage and third parties", body: ["Edu AI uses essential local storage to retain your preferences and work on the device. Domain-level measurement uses Cloudflare Web Analytics. The browser may also load the official Google AdSense tag associated with this domain to prepare and verify the platform; Edu AI does not configure ad placements or enable personalized advertising from its interface.", "If optional analytics or ads are enabled in the future, this page will be updated before those technologies are activated and a consent choice will be shown where required. Google may process technical information under its own policies when its tag loads."] },
  ...
  ru: {
    privacy: {
      ...
      sections: [
        ...
        { heading: "Cookies, хранилище и третьи стороны", body: ["Edu AI использует необходимое локальное хранилище для сохранения ваших настроек и работы на устройстве. Для измерения домена используется Cloudflare Web Analytics. Браузер также может загружать официальный тег Google AdSense, связанный с доменом, для подготовки и проверки платформы; Edu AI не настраивает рекламные блоки и не включает персонализированную рекламу через интерфейс.", "Если в будущем появятся необязательная аналитика или реклама, эта страница будет обновлена до их активации, а при необходимости будет показан выбор согласия. При загрузке своего тега Google может обрабатывать техническую информацию в соответствии со своими правилами."] },
