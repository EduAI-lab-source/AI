...
Después de publicar el refinamiento accesible, la pestaña **Preferencias** mostró nuevamente **Con enfoque de estudio** con su marca de selección y superficie activa. Cada opción expone además `aria-pressed`, mientras que el panel declara el estilo activo, por lo que lectores de pantalla y pruebas automatizadas pueden identificar la preferencia restaurada sin depender solo del color o del icono.

La puerta de Cloudflare ahora reenvía de forma explícita y exclusiva la consulta `auth.me` al backend, incluyendo la cookie de sesión cuando está presente, con CORS restringido a los tres orígenes públicos y `Access-Control-Allow-Credentials`. La ruta `https://textoavoz.xyz/api/oauth/callback` también se reenvía de forma estrecha al callback existente y ajusta la cookie de sesión a `.textoavoz.xyz`, de modo que puede utilizarse únicamente entre el sitio y `api.textoavoz.xyz`. Las llamadas de verificación devolvieron la respuesta del backend —no una ruta 404 del proxy— y el callback sin parámetros devolvió el `400` esperado del servidor. En desarrollo, la cabecera mostró el estado autenticado de la sesión preexistente mediante el control de cuenta, junto con el acceso voluntario al espacio de aprendizaje y el selector de idioma.

La versión estática publicada en `https://textoavoz.xyz/` conserva el uso local sin requerir una cuenta. Tras reforzar el renderizado público, la comprobación en una sesión sin cuenta mostró de forma estable el control **Guardar con cuenta**, con una explicación accesible de su propósito. El historial, el selector de idioma y el acceso a Mi espacio continúan disponibles sin redirecciones ni necesidad de iniciar sesión.

La comprobación directa del preflight en `https://api.textoavoz.xyz/api/trpc/workspace.accountSync` devolvió `204` con el origen `https://textoavoz.xyz`, credenciales habilitadas y los encabezados `authorization`, `content-type` y `x-trpc-batch` permitidos. Esto confirma que la sesión opcional puede iniciar solicitudes autenticadas a través del gateway sin abrir rutas adicionales.

La rama `gh-pages` se actualizó con la ficha contrastada de *La Teoterapia del amor*, atribuida a Néstor Chamorro Pesantes. Tras la carga del paquete actualizado, la biblioteca mostró correctamente la ficha entre *La culpa es de la vaca* y *Man's Search for Meaning*: etiqueta de lectura de espiritualidad cristiana, autor, explicación responsable, temas de amor, perdón y fe, y vínculo editorial HTTPS. La discrepancia de caché inicial quedó resuelta.
